...
sys.stderr.write("[run_model.py] Loading ONNX model...\n")
sys.stderr.flush()
warnings.filterwarnings('ignore') # Ignore all the warning messages in this tutorial
model = onnx.load('model2.onnx') # Load the ONNX file
sys.stderr.write("[run_model.py] ONNX model loaded successfully\n")
sys.stderr.flush()

sys.stderr.write("[run_model.py] Converting ONNX to TensorFlow...\n")
sys.stderr.flush()
tf_rep = prepare(model)
sys.stderr.write("[run_model.py] Conversion complete\n")
sys.stderr.flush()

sys.stderr.write("[run_model.py] Running inference...\n")
sys.stderr.flush()
output = tf_rep.run(input_data)[0].squeeze()
